...
pipeline_ids=$(ls ${CCL_DEVOPS_CONFIG_PIPELINE_ALBUM}/ | tr "_pipeline" "\n")

echo # empty line for readability
for id in $pipeline_ids
do
    # Check that pipeline folder includes a pipeline definition
    cli_pipeline_def_exists ${CCL_DEVOPS_CONFIG_PIPELINE_ALBUM} ${id}_pipeline

    # Get definition (really more of a config) of the pipeline we are running
    source "${CCL_DEVOPS_CONFIG_PIPELINE_ALBUM}/${id}_pipeline/pipeline_definition.sh"
