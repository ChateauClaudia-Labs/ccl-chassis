...
case "$1" in
  list)         ${CCL_DEVOPS_SERVICE_ROOT}/bin/commands/pipeline-list.sh ${@:2};; # $@ is an array of all arguments, so ${@:2} is a slice from 2nd elt onwards
  describe)     ${CCL_DEVOPS_SERVICE_ROOT}/bin/commands/pipeline-describe.sh ${@:2};;
  run)          ${CCL_DEVOPS_SERVICE_ROOT}/bin/commands/pipeline-run.sh ${@:2};;
